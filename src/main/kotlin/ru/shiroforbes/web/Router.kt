...
fun Routing.routes(
    ...
    authenticate("auth-session-admin-only") {
        get("/transactions/exercises/countryside") {
            val countryside = studentService!!.getGroup(GroupType.Countryside).sortedBy { student -> student.firstName() }
            call.respond(
                ThymeleafContent(
                    "exercises",
                    mapOf(
    ...
    authenticate("auth-session-admin-only") {
        get("/transactions/transactions/urban") {
            val urban = studentService!!.getGroup(GroupType.Urban).sortedBy { student -> student.firstName() }
            call.respond(
                ThymeleafContent(
                    "transactions",
                    mapOf(
